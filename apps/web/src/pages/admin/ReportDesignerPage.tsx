...
function sampleValue(key: string, row: number): string {
  if (/date|At$/i.test(key))
    return ['08/04/2026', '09/04/2026', '10/04/2026', '10/04/2026'][row % 4];
  ...
  if (/nature|type|direction|sens|category|journal/i.test(key))
    return ['Vente', 'Achat', 'Banque', 'Caisse'][row % 4];
  if (/label|libellé|description|comment/i.test(key))
    return ['Vente comptoir', 'Fournitures', 'Versement bancaire', 'Frais divers'][row % 4];
  if (/account.*num|accountNumber/i.test(key))
    return ['570000', '411000', '512000', '601000'][row % 4];
  if (/account.*label|accountLabel/i.test(key))
    return ['Caisse', 'Clients', 'Banque', 'Achats'][row % 4];
  ...
  if (
    /amount|debit|credit|balance|entry|exit|total|cash|check|transfer|mobile|variation|subtotal|cumul|solde|variance|opening|closing|theoretical|actual/i.test(
      key,
    )
  )
...
export default function ReportDesignerPage() {
  ...
  const handleDragOver = (e: React.DragEvent, k: string) => {
    e.preventDefault();
    dragOverItem.current = k;
    setFieldDropTarget(k);
  };
  const handleDrop = () => {
    if (
      !selectedReportId ||
    ...
    const fi = keys.indexOf(dragItem.current),
      ti = keys.indexOf(dragOverItem.current);
    if (fi < 0 || ti < 0) return;
    keys.splice(fi, 1);
    keys.splice(ti, 0, dragItem.current);
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] text-gray-500">
            {r.fields.filter((f) => f.visible).length}/{r.fields.length} colonnes
          </div>
        </div>
  ...
  const CategoryGroup = ({
    ...
    return (
      <div>
        <button
          onClick={() => setExpandedCategory(exp ? null : cat)}
        ...
        >
          ...
          {label}
          <Badge className="ml-auto bg-white/10 text-gray-400 text-[9px] px-1">
            {items.length}
          </Badge>
        </button>
        {exp && (
    ...
    if (!selectedReport)
      return (
        <div className="flex-1 flex flex-col items-center justify-center text-gray-500 gap-3">
          ...
          <p className="text-sm">
            {t('reportDesigner.selectReport', 'Sélectionnez un état à personnaliser')}
          </p>
          <p className="text-xs text-gray-600">
            Cliquez sur un élément de l'état pour le modifier directement
          </p>
        </div>
      );
          ...
          <div
            className={cn(
              ...
              isSelected
                ...
                : 'bg-gray-200/80 text-gray-500 opacity-0 group-hover/band:opacity-100',
            )}
          >
        ...
        case 'header':
          return wrapBand(
            <div
              ...
              style={{
                padding: `${pg.marginTop}px ${pg.marginLeft}px 16px`,
                fontFamily: h.companyStyle.fontFamily,
              }}
            >
              {h.showCompanyName && (
                <div
                  style={{
                    fontSize: h.companyStyle.fontSize,
                    fontWeight: h.companyStyle.fontWeight,
                    color: h.companyStyle.color,
                    fontStyle: h.companyStyle.fontStyle,
                    textAlign: h.companyStyle.textAlign,
                  }}
                >
                  SOCIÉTÉ ABC SARL
                </div>
              )}
              {h.showCompanyAddress && (
                <div className="text-xs text-gray-500">
                  123 Avenue du Commerce, Brazzaville, Congo
                </div>
              )}
              {h.showCompanyPhone && (
                <div className="text-xs text-gray-500">Tél: +242 06 123 4567</div>
              )}
              {h.showCompanyTaxId && (
                <div className="text-xs text-gray-500">RCCM: CG-BZV-01-2024-A1234</div>
              )}
              ...
              {(!h.showCompanyName ||
                ...
                <div className="flex justify-center gap-2 mt-1 flex-wrap">
                  {!h.showCompanyName && (
                    <span className="text-[9px] text-gray-300 line-through font-sans">Nom</span>
                  )}
                  {!h.showCompanyAddress && (
                    <span className="text-[9px] text-gray-300 line-through font-sans">Adresse</span>
                  )}
                  {!h.showCompanyPhone && (
                    <span className="text-[9px] text-gray-300 line-through font-sans">Tél</span>
                  )}
                  {!h.showCompanyTaxId && (
                    <span className="text-[9px] text-gray-300 line-through font-sans">Fiscal</span>
                  )}
                </div>
              )}
              ...
              <div className="border-t border-gray-300 mt-3 pt-3">
                <div
                  style={{
                    fontSize: h.titleStyle.fontSize,
                    fontWeight: h.titleStyle.fontWeight,
                    color: h.titleStyle.color,
                    fontStyle: h.titleStyle.fontStyle,
                    textAlign: h.titleStyle.textAlign,
                ...
                {h.customSubtitle && (
                  <div
                    style={{
                      fontSize: h.subtitleStyle.fontSize,
                      color: h.subtitleStyle.color,
                      fontStyle: h.subtitleStyle.fontStyle,
                      textAlign: h.subtitleStyle.textAlign,
                    }}
                    className="mt-0.5"
                  >
                    {h.customSubtitle}
                ...
                <div className="text-xs text-gray-400 mt-0.5">
                  {rp.category === 'daily'
                    ? 'Journée du 10/04/2026'
                    : 'Période du 01/04/2026 au 10/04/2026'}
                </div>
              </div>
            </div>,
          );
                  ...
                  <div className="flex flex-wrap gap-2">
                    {visibleKpis.map((kpi) => {
                      ...
                      return (
                        <div
                          ...
                          onDragStart={(e) => {
                            e.stopPropagation();
                            handleKpiDragStart(e, kpi.key);
                          }}
                          onDragOver={(e) => {
                            e.stopPropagation();
                            handleKpiDragOver(e, kpi.key);
                          }}
                          onDrop={(e) => {
                            e.stopPropagation();
                            handleKpiDrop(kpi.key);
                          }}
                          ...
                          className={cn(
                            'flex-1 min-w-[100px] rounded-lg p-2.5 text-center cursor-pointer transition-all border-2 relative group/kpi',
                            isSel
                              ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-200'
                              : 'border-blue-100 bg-blue-50/70 hover:border-blue-300',
                        ...
                        >
                          ...
                          {isDrop && (
                            <div className="absolute -left-[3px] top-1 bottom-1 w-[3px] bg-green-500 rounded-full shadow-sm shadow-green-500/50" />
                          )}
                          <div
                            style={{
                              fontSize: kpi.style.fontSize,
                              fontFamily: kpi.style.fontFamily,
                              fontWeight: kpi.style.fontWeight as string,
                              color: kpi.style.color,
                              fontStyle: kpi.style.fontStyle,
                              textAlign: kpi.style.textAlign,
                            }}
                          >
                            {kpi.customLabel || kpi.label}
                          </div>
                          <div className="text-sm font-bold text-blue-900 mt-0.5">
                            {sampleKpiValue(kpi.key)}
                          </div>
                        </div>
                      );
                    })}
                    {hiddenKpis.map((kpi) => (
                      <div
                        ...
                        className={cn(
                          'flex-1 min-w-[80px] rounded-lg p-2 text-center cursor-pointer border-2 border-dashed border-gray-200 bg-gray-50/50 opacity-40 hover:opacity-60',
                          kpiDropTarget === kpi.key && 'border-green-400 opacity-80',
                        )}
                      >
                        <div className="text-[9px] text-gray-400 font-sans line-through">
                          {kpi.customLabel || kpi.label}
                        </div>
          ...
          return wrapBand(
            ...
            >
              <div
                className={cn(
                  'overflow-hidden',
                  tbl.showBorders ? 'border border-gray-300 rounded' : '',
                )}
              >
                      ...
                      {sortedFields.map((field) => {
                        ...
                        if (!field.visible)
                          return (
                            <th
                              ...
                              className={cn(
                                'w-3 min-w-[12px] max-w-[12px] px-0 py-2 cursor-pointer',
                                isSel ? 'bg-blue-100' : 'opacity-40 hover:opacity-70',
                              )}
                        ...
                        return (
                          <th
                            ...
                            className={cn(
                              'group/col px-2 py-2 cursor-pointer transition-all select-none relative',
                              tbl.showBorders && 'border-b border-gray-300',
                              isSel
                                ? 'bg-blue-100 text-blue-800 shadow-inner'
                                : 'hover:bg-blue-50/60',
                              fieldDropTarget === field.key &&
                                dragItem.current !== field.key &&
                                'bg-green-50',
                            )}
                            style={{
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1">
                              <GripVertical
                                size={10}
                                className="text-gray-300 cursor-grab active:cursor-grabbing flex-shrink-0 opacity-0 group-hover/col:opacity-100 transition-opacity"
                              />
                        ...
                        {sortedFields.map((field) => {
                          ...
                          if (!field.visible)
                            return (
                              <td
                                key={field.key}
                                className="w-3 min-w-[12px] max-w-[12px] px-0 bg-gray-50/30"
                              />
                            );
                          return (
                            <td
                              ...
                              style={{
                                ...
                                fontWeight: field.style.fontWeight as string,
                  ...
                  {tbl.showTotalsRow && (
                    <tfoot>
                      <tr
                        ...
                        style={{ backgroundColor: tbl.headerBg }}
                      >
                        {sortedFields.map((field) => {
                          if (!field.visible)
                            return (
                              <td key={field.key} className="w-3 min-w-[12px] max-w-[12px] px-0" />
                            );
                          const isNum =
                            /amount|debit|credit|balance|entry|exit|total|cash|subtotal|variation/i.test(
                              field.key,
                            );
                          const isFirst =
                            sortedFields.findIndex((f) => f.visible) ===
                            sortedFields.indexOf(field);
                          return (
                            <td
                              key={field.key}
              ...
              {hiddenFields.length > 0 && (
                <div className="mt-1.5 flex items-center gap-1 text-[9px] text-gray-400 font-sans">
                  ...
                  {hiddenFields.length} colonne(s) masquée(s):{' '}
                  {hiddenFields.map((f) => f.customLabel || f.label).join(', ')}
                </div>
              )}
            </div>,
          );
        ...
        case 'footer':
          return wrapBand(
            <div
              ...
              className={cn(
                'text-xs cursor-pointer transition-all relative',
                selection?.kind === 'footer' && 'bg-blue-50/60',
              )}
              style={{
                padding: `20px ${pg.marginRight}px ${pg.marginBottom}px ${pg.marginLeft}px`,
                fontFamily: ft.style.fontFamily,
                fontSize: ft.style.fontSize,
                color: ft.style.color,
              }}
            >
              <div className="flex justify-between items-start">
                {ft.showEstabliPar ? (
                  <div>
                    <span className="font-semibold text-gray-700">Établi par :</span>{' '}
                    <span className="text-gray-500">Jean Dupont</span>
                  </div>
                ) : (
                  <div className="font-sans text-[10px] text-gray-300 line-through italic">
                    "Établi par" (masqué)
                  </div>
                )}
                {ft.showVerifiePar ? (
                  <div>
                    <span className="font-semibold text-gray-700">Vérifié par :</span>{' '}
                    <span className="text-gray-500">{ft.verifiedByLabel || 'Le Directeur'}</span>
                  </div>
                ) : (
                  <div className="font-sans text-[10px] text-gray-300 line-through italic">
                    "Vérifié par" (masqué)
              ...
              {ft.showTimestamp ? (
                ...
                <div className="text-right mt-2 font-sans text-[10px] text-gray-300 line-through italic">
                  Date & heure (masqué)
                </div>
              )}
            </div>,
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            {isModified(rp.id) && (
              <Badge className="bg-brand-gold/15 text-brand-gold text-[10px]">
                {t('reportDesigner.modified', 'modifié')}
              </Badge>
            )}
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-500">
              {visibleFields.length}/{sortedFields.length} col · {visibleKpis.length}/
              {rp.kpis.length} KPI
            </span>
            ...
            {isModified(rp.id) && (
              ...
              >
                ...
                {t('reportDesigner.resetReport', 'Réinitialiser')}
              </Button>
            )}
          </div>
        </div>
        ...
        <div className="max-w-[820px] mx-auto mt-3 text-center">
          <p className="text-[10px] text-gray-500">
            Glissez les sections pour réordonner · Glissez les colonnes et KPIs pour les déplacer ·
  ...
  const PropertiesPanel = () => {
    ...
    if (!selection) {
      ...
      return (
        <div className="p-4 space-y-4">
          ...
          <p className="text-[10px] text-gray-500">
            {DATA_SOURCE_FIELDS.length} champs disponibles dans {dsGroups.length} sources. Ajoutez
            des colonnes ou KPIs depuis ces sources.
          </p>
          <div className="space-y-1">
            {dsGroups.map((g) => {
              const count = DATA_SOURCE_FIELDS.filter((f) => f.group === g).length;
              const usedCount = DATA_SOURCE_FIELDS.filter(
                (f) =>
                  f.group === g &&
                  (selectedReport.fields.some((ef) => ef.key === f.key) ||
                    selectedReport.kpis.some((ek) => ek.key === f.key)),
              ).length;
              return (
                <div
                  key={g}
          ...
          <p className="text-[9px] text-gray-600 text-center">
            Cliquez sur un élément de l'aperçu pour personnaliser son style
          </p>
        </div>
      );
    }
    ...
    const rp = selectedReport;
    const CloseBtn = () => (
      <button onClick={clearSelection} className="p-1 rounded hover:bg-white/10 text-gray-500">
        <X size={14} />
      </button>
    );
          ...
          <div>
            ...
            <div className="flex gap-1">
              {(['portrait', 'landscape'] as const).map((o) => (
                <button
                  key={o}
                  onClick={() => updatePage(rp.id, { orientation: o })}
                  className={cn(
              ...
              {(
                ...
                <div key={k} className="flex items-center gap-1">
                  ...
                  <Input
                    ...
                    value={pg[k]}
                    onChange={(e) =>
                      updatePage(rp.id, { [k]: Math.max(0, Math.min(80, Number(e.target.value))) })
                    }
              ...
              {[...rp.sections]
                ...
                .map((sec, idx, arr) => {
                  ...
                  return (
                    <div
                      key={sec.type}
                      className="flex items-center gap-1.5 px-2 py-1.5 rounded hover:bg-white/5 group/secitem"
                    >
                      ...
                      <div className="flex items-center gap-0.5 opacity-0 group-hover/secitem:opacity-100 transition-opacity">
                        <button
                          onClick={moveSecUp}
                          disabled={idx === 0}
                          className="p-0.5 rounded hover:bg-white/10 text-gray-500 disabled:opacity-30"
                        >
                          <ArrowUp size={10} />
                        </button>
                        <button
                      ...
                      <button
                        onClick={() => updateSection(rp.id, sec.type, { visible: !sec.visible })}
                        className={cn(
                          'p-0.5 rounded transition-colors',
                          sec.visible ? 'text-green-400' : 'text-gray-600',
                        )}
                      >
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            <label className="block text-[10px] text-gray-500 uppercase mb-1">
              Position ({secIdx + 1}/{sortedSecs.length})
            </label>
            <div className="flex gap-1">
              <Button
                variant="outline"
                ...
                disabled={secIdx === 0}
                onClick={() => moveSec('up')}
                className="border-white/10 text-xs h-6 flex-1"
              >
              ...
              <Button
                variant="outline"
                size="sm"
                disabled={secIdx === sortedSecs.length - 1}
                onClick={() => moveSec('down')}
                className="border-white/10 text-xs h-6 flex-1"
              >
          ...
          <p className="text-[10px] text-gray-500">
            Glissez la section directement sur l'aperçu ou utilisez les boutons ci-dessus. Cliquez
            sur les éléments à l'intérieur pour modifier leur style.
          </p>
        </div>
      );
    }
        ...
        <div className="p-4 space-y-3 overflow-y-auto max-h-full">
          ...
          <div className="space-y-1">
            {toggles.map(({ key, label }) => (
              ...
              >
                <div
                  onClick={() => updateHeader(rp.id, { [key]: !hd[key] })}
                  className={cn(
          ...
          <div className="space-y-2 pt-2 border-t border-white/10">
            ...
            <Input
              value={hd.customTitle || ''}
              onChange={(e) => updateHeader(rp.id, { customTitle: e.target.value || undefined })}
              placeholder={rp.name}
              className="bg-white/5 border-white/10 text-xs h-7"
            />
            <label className="block text-[10px] text-gray-500 uppercase">Sous-titre</label>
            <Input
              value={hd.customSubtitle || ''}
              onChange={(e) => updateHeader(rp.id, { customSubtitle: e.target.value || undefined })}
          ...
          <StyleEditor
            ...
            style={hd.titleStyle}
            onChange={(s) => updateHeader(rp.id, { titleStyle: { ...hd.titleStyle, ...s } })}
          />
          <StyleEditor
            label="Style société"
            style={hd.companyStyle}
            onChange={(s) => updateHeader(rp.id, { companyStyle: { ...hd.companyStyle, ...s } })}
          />
        </div>
      );
    }
      ...
      return (
        <div className="p-4 space-y-3">
          ...
          <div>
            ...
            <Input
              value={kpi.customLabel || kpi.label}
              onChange={(e) =>
                updateKpi(rp.id, kpi.key, { customLabel: e.target.value || undefined })
              }
              className="bg-white/5 border-white/10 text-xs h-7"
            />
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <div
              onClick={() => updateKpi(rp.id, kpi.key, { visible: !kpi.visible })}
              className={cn(
                'w-4 h-4 rounded border-2 flex items-center justify-center cursor-pointer',
          ...
          <StyleEditor
            label="Style"
            style={kpi.style}
            onChange={(s) => updateKpiStyle(rp.id, kpi.key, s)}
          />
          {/* Quick nav */}
          <div className="pt-2 border-t border-white/10">
            ...
            <div className="space-y-0.5">
              {rp.kpis.map((k) => (
                <button
                  key={k.key}
                  onClick={() => select('kpi', k.key)}
                  className={cn(
                    'w-full text-left text-[11px] px-2 py-0.5 rounded flex items-center gap-2',
                    k.key === selection.key
      ...
      return (
        <div className="p-4 space-y-3">
          ...
          <div className="space-y-1.5">
            {(
              [
                ...
                keyof Pick<typeof tbl, 'showBorders' | 'stripedRows' | 'showTotalsRow'>,
                string,
              ][]
            ).map(([key, label]) => (
              <label
              ...
              >
                <div
                  onClick={() => updateTable(rp.id, { [key]: !tbl[key] })}
                  className={cn(
                    'w-4 h-4 rounded border-2 flex items-center justify-center cursor-pointer',
          ...
          <div>
            ...
            <Input
              value={tbl.totalsLabel}
              onChange={(e) => updateTable(rp.id, { totalsLabel: e.target.value })}
              className="bg-white/5 border-white/10 text-xs h-7"
            />
          </div>
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-1">
                {['#f3f4f6', '#e5e7eb', '#dbeafe', '#fef9c3', '#dcfce7', '#ffffff'].map((c) => (
                  <button
                    key={c}
                    onClick={() => updateTable(rp.id, { headerBg: c })}
                    className={cn(
                      'w-5 h-5 rounded border-2',
                      tbl.headerBg === c ? 'border-brand-gold' : 'border-transparent',
                    )}
                    style={{ backgroundColor: c }}
                  />
                ))}
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-1">
                {['#374151', '#111827', '#1e40af', '#991b1b', '#166534'].map((c) => (
                  <button
                    key={c}
                    onClick={() => updateTable(rp.id, { headerColor: c })}
                    className={cn(
                      'w-5 h-5 rounded border-2',
                      tbl.headerColor === c ? 'border-brand-gold' : 'border-transparent',
                    )}
                    style={{ backgroundColor: c }}
                  />
                ))}
          ...
          <p className="text-[10px] text-gray-500 italic">
            Cliquez sur une colonne du tableau pour modifier son style individuel
          </p>
        </div>
      );
    }
        ...
        <div className="p-4 space-y-3 overflow-y-auto max-h-full">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                updateField(rp.id, field.key, { customLabel: e.target.value || undefined })
              }
              className="bg-white/5 border-white/10 text-xs h-7"
            />
            {field.customLabel && field.customLabel !== field.label && (
              <span className="text-[9px] text-gray-500 mt-0.5 block">
                par défaut: {field.label}
              </span>
            )}
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
            <div
              onClick={() => updateField(rp.id, field.key, { visible: !field.visible })}
              className={cn(
                'w-4 h-4 rounded border-2 flex items-center justify-center cursor-pointer',
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                value={field.width || 0}
                onChange={(e) =>
                  updateField(rp.id, field.key, { width: Number(e.target.value) || undefined })
                }
          ...
          <StyleEditor
            label="Style de la colonne"
            style={field.style}
            onChange={(s) => updateFieldStyle(rp.id, field.key, s)}
          />
          {/* Remove custom field */}
          {!DEFAULT_REPORTS.find((d) => d.id === rp.id)?.fields.some(
            (df) => df.key === field.key,
          ) && (
            <button
              onClick={() => {
                removeField(rp.id, field.key);
                clearSelection();
              }}
              className="flex items-center gap-1.5 text-[11px] text-red-400 hover:text-red-300 hover:bg-red-500/10 px-2 py-1 rounded transition-colors w-full"
            >
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="space-y-1">
            {toggles.map(({ key, label }) => (
              ...
              >
                <div
                  onClick={() => updateFooter(rp.id, { [key]: !ftr[key] })}
                  className={cn(
                    'w-4 h-4 rounded border-2 flex items-center justify-center cursor-pointer',
          ...
          <div>
            ...
            <Input
              value={ftr.verifiedByLabel || ''}
              onChange={(e) =>
                updateFooter(rp.id, { verifiedByLabel: e.target.value || undefined })
              }
          ...
          <StyleEditor
            label="Style pied de page"
            style={ftr.style}
            onChange={(s) => updateFooter(rp.id, { style: { ...ftr.style, ...s } })}
          />
        </div>
      );
    }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-white/10">
        <div>
          <h1 className="text-lg font-bold text-gray-100">
            {t('reportDesigner.title', "Éditeur d'états imprimables")}
          </h1>
          <p className="text-[10px] text-gray-400 mt-0.5">
            Conception visuelle DFM — Cliquez directement sur l'aperçu PDF pour personnaliser la
            structure, les champs et la mise en page
          </p>
        </div>
        <div className="flex items-center gap-2">
          {/* Sync status */}
          ...
          >
            <CloudUpload size={12} className="mr-1.5" />
            Tout enregistrer
          </Button>
          ...
          >
            ...
            {t('reportDesigner.resetAll', 'Tout réinitialiser')}
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-56 border-r border-white/10 overflow-y-auto p-2 space-y-2 flex-shrink-0">
          <CategoryGroup
            cat="daily"
            label={t('reportDesigner.dailyReports', 'États journaliers')}
            items={dailyReports}
          />
          <CategoryGroup
            cat="period"
            label={t('reportDesigner.periodReports', 'États périodiques')}
            items={periodReports}
          />
          ...
          {selectedReport && (
            <div className="border-t border-white/10 pt-2 mt-2">
              ...
              <div className="mt-1 space-y-0.5 max-h-60 overflow-y-auto">
                {(() => {
                  const groups = [
                    ...new Set(DATA_SOURCE_FIELDS.map((f) => f.group)),
                  ] as DataSourceGroup[];
                  return groups.map((g) => {
                    ...
                    return (
                      <details key={g} className="group/ds">
                        <summary className="flex items-center gap-1.5 px-2 py-1 rounded hover:bg-white/5 cursor-pointer text-[11px] text-gray-400 select-none">
                          <ChevronRight
                            size={10}
                            className="transition-transform group-open/ds:rotate-90 text-gray-600"
                          />
                          <span className="truncate flex-1">{DATA_SOURCE_LABELS[g]}</span>
                          <Badge
                            variant="outline"
                            className="text-[8px] px-1 py-0 border-white/10 text-gray-500"
                          >
                            {fields.length}
                          </Badge>
                        </summary>
                        <div className="ml-4 mt-0.5 space-y-0">
                          {fields.map((f) => {
                            ...
                            const alreadyField = selectedReport.fields.some(
                              (ef) => ef.key === f.key,
                            );
                            ...
                            return (
                              ...
                              >
                                ...
                                {(alreadyField || alreadyKpi) && (
                                  <Check size={8} className="text-green-500/50" />
                                )}
                              </div>
                            );
                          })}
      ...
      {showFieldPicker &&
        ...
        (() => {
          ...
          const filteredFields = DATA_SOURCE_FIELDS.filter(
            (f) => fieldPickerGroup === 'all' || f.group === fieldPickerGroup,
          ).filter(
            (f) =>
              !fieldPickerSearch ||
          ...
          return (
            ...
            >
              <div
                className="bg-gray-900 border border-white/10 rounded-xl w-full max-w-2xl max-h-[80vh] flex flex-col"
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3 min-h-0">
                  {filteredFields.length === 0 ? (
                    ...
                    (() => {
                      ...
                      const byGroup = filteredFields.reduce<Record<string, DataSourceField[]>>(
                        (acc, f) => {
                          (acc[f.group] ??= []).push(f);
                          return acc;
                        },
                            ...
                            {fields.map((f) => {
                              ...
                              return (
                                ...
                                >
                                  <Icon
                                    size={12}
                                    className={already ? 'text-gray-600' : 'text-brand-gold/70'}
                                  />
                                  <span className="truncate flex-1">{f.label}</span>
                                  {already ? (
                ...
                <div className="flex items-center justify-between px-5 py-3 border-t border-white/10">
                  <span className="text-[10px] text-gray-500">
                    {filteredFields.length} champ{filteredFields.length > 1 ? 's' : ''} disponible
          ...
          <div className="bg-gray-900 border border-white/10 rounded-xl p-6 w-full max-w-sm space-y-4">
            <div className="flex items-center gap-3 text-yellow-400">
              ...
              <h3 className="font-bold">
                {t('reportDesigner.confirmResetTitle', 'Réinitialiser cet état ?')}
              </h3>
            </div>
            <p className="text-sm text-gray-300">
              {t(
                'reportDesigner.confirmResetMsg',
                'La configuration de « {{name}} » sera rétablie.',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-gray-900 border border-white/10 rounded-xl p-6 w-full max-w-sm space-y-4">
            <div className="flex items-center gap-3 text-red-400">
              <AlertTriangle size={20} />
              <h3 className="font-bold">
                {t('reportDesigner.confirmResetAllTitle', 'Tout réinitialiser ?')}
              </h3>
            </div>
            <p className="text-sm text-gray-300">
              {t(
                'reportDesigner.confirmResetAllMsg',
                'Toutes les personnalisations seront perdues.',
              )}
            </p>
            <div className="flex justify-end gap-2">
              <Button variant="outline" size="sm" onClick={() => setShowResetAllConfirm(false)}>
                {t('common.cancel', 'Annuler')}
              </Button>
              <Button
                size="sm"
