      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            {monthlyTrend.length > 0 ? (
                <ResponsiveContainer width="100%" height={288}>
                  <AreaChart data={monthlyTrend}>
                    ...
                    <Area type="monotone" dataKey="amount" name={t('dashboard.expenses')} stroke="#EA761D" strokeWidth={2.5}
                      fill="url(#expenseGrad)" dot={{ r: 3, fill: '#EA761D', strokeWidth: 0 }} />
                  </AreaChart>
                </ResponsiveContainer>
            ) : <EmptyChart />}
          </CardContent>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <button onClick={() => navigate('/expenses')} className="flex items-center gap-1 text-sm text-brand-gold hover:text-brand-gold-dark">
            ...
            {recentExpenses.length > 0 ? (
              <div className="space-y-3">
                {recentExpenses.map((exp) => (
                  <button key={exp.id} onClick={() => navigate(`/expenses/${exp.id}`)}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-500">{exp.categoryName}{exp.beneficiary ? ` — ${exp.beneficiary}` : ''}</p>
                    </div>
                    <div className="ml-3 flex items-center gap-3">
                      <Badge variant={STATUS_VARIANTS[exp.status] ?? 'default'} className="text-[10px]">
                        {STATUS_LABELS[exp.status] ?? exp.status}
                      </Badge>
                      <span className="text-sm font-semibold text-gray-900 whitespace-nowrap">{formatCFA(exp.amount)}</span>
                    </div>
                  </button>
                ))}
              </div>
