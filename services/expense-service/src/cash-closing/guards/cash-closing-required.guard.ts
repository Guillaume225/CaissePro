 ...
 *
 * Usage: add @UseGuards(CashClosingRequiredGuard) on expense/advance controllers
 * or register as APP_GUARD and use @SkipCashClosingCheck() to opt out.
 */
@Injectable()
export class CashClosingRequiredGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Only block write operations (POST, PATCH, PUT, DELETE)
