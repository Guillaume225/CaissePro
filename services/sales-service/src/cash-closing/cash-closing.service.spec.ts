...
describe('CashClosingService (sales)', () => {
  ...
  describe('getCurrent', () => {
    it('should return the current open register with live totals', async () => {
      ...
      const payQb = {
        ...
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
        getRawOne: jest.fn().mockResolvedValue({ total: '35000' }),
      };
      ...
      expect(result.openingBalance).toBe(100000);
      expect(result.totalEntries).toBe(35000);
      expect(result.totalExits).toBe(0);
      expect(result.theoreticalBalance).toBe(135000); // 100000 + 35000 - 0
    });
  ...
  describe('close', () => {
    it('should close the register with matching balance', async () => {
      ...
      // theoretical = 100000 + 50000 - 0 = 150000
      const result = await service.close({ actualBalance: 150000 }, mockUser);

      expect(result.status).toBe(CashDayStatus.CLOSED);
      expect(result.theoreticalBalance).toBe(150000);
      expect(result.actualBalance).toBe(150000);
    ...
    it('should compute variance correctly', async () => {
      ...
      // theoretical = 100000 + 40000 = 140000, actual = 138000, variance = -2000
      const result = await service.close({ actualBalance: 138000 }, mockUser);
    ...
    it('should accept large variance with comment', async () => {
      ...
      const result = await service.close(
        { actualBalance: 112000, comment: 'Billet manquant' },
        mockUser,
      );
